	...
	for (size_t k = 0; k < nworks; ++k)
	{
		vthread[k] = std::thread([&, k]() {
			...
			for (size_t j = 0; j < rounds; ++j)
			{
				...
				malloc_costtime += (size_t)std::chrono::duration_cast<std::chrono::milliseconds>(end1 - begin1).count();
				free_costtime += (size_t)std::chrono::duration_cast<std::chrono::milliseconds>(end2 - begin2).count();
			}
		});
	}
